...
enum WalletFeature {
    FEATURE_BASE = 10500, // the earliest version new wallets suctdcoinrts (only useful for getinfo's clientversion output)

    FEATURE_WALLETCRYPT = 30000, // wallet encryption
    FEATURE_COMPRPUBKEY = 31000, // compressed public keys

    FEATURE_LATEST = 33331
};
...
public:
    ...
    //! check whether we are allowed to upgrade (or already suctdcoinrt) to the named feature
    bool CanSuctdcoinrtFeature(enum WalletFeature wf)
    {
    ...
    /// Get 1000CTD output and keys which can be used for the Masternode
