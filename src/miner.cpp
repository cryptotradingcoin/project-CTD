...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CTDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CTDMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CTDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ctdcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CTDMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CTDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCTDMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCTDMiner() exception");
    }

    LogPrintf("ThreadCTDMiner exiting\n");
}
